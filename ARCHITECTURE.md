...
- **Member Directory & Spotlight**: Directory of community members, highlighting a carousel of spotlighted members. The spotlight system (v2) is driven by weekly ISO-week keys, automated birthday prioritization with 30-day retroactive grace period for new members, a round-robin fallback queue, and manual admin override. Supports multiple simultaneous spotlights — when two or more members qualify (e.g., same-month birthdays), all are shown in an auto-scrolling carousel with dot indicators. Selection is handled by a daily Cloud Scheduler function (`rotateSpotlightScheduled`, 9:05 AM ET); the public `getMemberSpotlight` endpoint is read-only. Core logic lives in `functions/src/apis/spotlight.ts`.
    - **Contact Info Gating**: Email, phone, and birthday fields are gated behind confirmed membership status (`userProfileData?.isMember == true && LoginUIState.Authenticated`). Non-members see an italic placeholder: "Contact details visible to members only". Name, role, profession, company, interests, and wines remain public.
    - **`MemberAvatar` Composable** (`MembersPage.kt`): Reusable avatar with initials fallback. Displays `AsyncImage` when `profilePictureUrl` is present; falls back to a `CircleShape` box with the member's initials (first letter of each name word, max 2 chars) on a translucent `WdwOrange` background. Font size scales at 35% of the avatar diameter. Used in both `MemberCard` (80dp) and `MemberDetailPane` (160dp), and also in chat @mention suggestion rows (28dp). Handles image load failures via `onError` callback.
    - **Emeritus Section**: Members with `isEmeritus: true` (set via admin CRUD) are partitioned into a dedicated "Emeritus" section at the bottom of the Members page, after Guests. Emeritus members are separated from their regular membership type (Leader/Member/Guest). Cards use the same visual style with a `🎖 Emeritus` badge (WdwOrange accent on translucent background). The section is only rendered if emeritus members exist. Model: `Member.kt` includes `val isEmeritus: Boolean = false` (defaults to false for backward compatibility). Logic: `MembersPageViewModel.loadMembers()` checks `isEmeritus` before `memberType`.
...
- **UI & Interaction Polish**:
    ...
    - **Deleted User Channels**: DM channels where the other participant's account was deleted display "Deleted User" instead of the raw `deleted-user-*` ID. Detection is in `_mapChannelData` in `stream-bridge.js`.
    - **Delete DM Conversations**: Users can remove DM channels from their sidebar. A 🗑️ trash icon appears on hover (desktop) with a dropdown confirmation menu. Uses Stream's `channel.hide()` API — the channel will reappear if the other user sends a new message. Callback chain: `MessagingScreen` → `ChatLayout` → `ChannelSidebar` → `ChannelItem(onDelete)`. Community channels are not deletable (no `onDelete` callback passed).
...
- **Vino AI Bot** (`vino-bot`):
    ...
    - **UI Treatment**: Vino bot messages are visually distinct from regular messages:
     ...
     - **@mention autocomplete** (`MentionSuggestionPopup`): Typing `@` in any chat input shows a filtered popup of all channel members + Vino bot. Members are sourced from `LocalMembers` CompositionLocal. Each row shows `MemberAvatar` (28dp) + name. Clicking or pressing **Enter** selects the top suggestion and inserts `@Name ` into the text field. The popup auto-filters as the user types after `@`.
...
- **Scheduled Spotlight Rotation**: `rotateSpotlightScheduled` runs daily at 9:05 AM ET via Cloud Scheduler. The `getMemberSpotlight` public endpoint is now strictly read-only (no side-effect writes). Selection logic in `spotlight.ts` uses ISO-week period keys with 30-day TTL.
...
## Onboarding Enforcement & Welcome System

### Overview
A four-pillar system for onboarding new users, gated behind the `onboardingEnforcement` and `vinoWelcomePosts` feature flags.

### Pillar 1 — Enforcement Gating
When `onboardingEnforcement` is ON, users with incomplete profiles (missing `name`, `phone`, or `isVerified`) are blocked from:

| Surface | Behavior |
|---------|----------|
| **Messaging** | Full-screen overlay ("🔒 Complete your profile to start chatting") replaces the chat UI |
| **RSVP** | `window.alert()` before opening RSVP form |
| **Profile "Skip for now"** | Hidden for new users — forces completion |
| **App-wide banner** | Orange WdwOrange banner below TopNavBar linking to Profile page |
...
- If the user's profile is still incomplete, `chatWithBot` injects `[ONBOARDING_MODE]` context into the Gemini prompt, listing missing fields.
- Vino conversationally collects profile data (name, phone, profession, company, interests, favoriteWines).
- Extracted data is parsed from `ACTION:PROFILE_UPDATE` markers and persisted to Firestore automatically.

### Pillar 4 — Profile Field Parity
`UserProfileData` now includes fields previously only available on the `Member` model:

| Field | Type | Source |
|-------|------|--------|
| `profession` | `String?` | User-editable on Profile page |
| `company` | `String?` | User-editable on Profile page |
| `interests` | `List<String>?` | User-editable, comma-separated input |
| `favoriteWines` | `List<String>?` | User-editable, comma-separated input |
| `profileComplete` | `Boolean?` | Server-computed (name + phone + isVerified) |

`isOnboardingComplete` is a **client-computed** property: `!name.isNullOrEmpty() && !phone.isNullOrEmpty() && isVerified == true`.

### Profile Edit UI
`ProfileEditSection` now has two grouped sections:
